...
plt.xlabel('Features')
#plt.title('Random Forest Feature Importance as Percentage of Total')
plt.xticks(rotation=45, ha='center')

# Adding "RF" to the top right
plt.text(0.95, 0.95, 'BRF', horizontalalignment='right', verticalalignment='top', transform=ax.transAxes, fontsize=12, fontweight='bold')
...
# Convert to percentages
importances_in_percent = importances * 100

# Pair feature names with their importance scores
feature_importance_pairs = zip(feature_names, importances_in_percent)

# Sort the features by importance
sorted_feature_importances = sorted(feature_importance_pairs, key=lambda x: x[1], reverse=True)

# Print out the feature importances in percentage
print("Feature importances (%):")
for feature, importance_percent in sorted_feature_importances:
    print(f"{feature}: {importance_percent:.2f}%")
...
def plot_MVAoutput(y_truth, y_score, nbins=100):
    ...
    plt.ylabel('Entries')
    plt.legend()
    plt.show() #I added to see the plot
